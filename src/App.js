...
function RadarChart({ scores, color, size }) {
  color = color || "#e85d04";
  size = size || 220;
  const cx = size/2;
  const cy = size/2;
  ...
  const gridPts = function(l) {
    return CRITERIA.map(function(_,i) {
      const rad = r*l/5;
      return (cx+rad*Math.cos(angle(i)))+","+(cy+rad*Math.sin(angle(i)));
    }).join(" ");
  };
  const dataPts = CRITERIA.map(function(c,i) {
    const v = (scores[c.key]||0)/10;
    return (cx+r*v*Math.cos(angle(i)))+","+(cy+r*v*Math.sin(angle(i)));
  }).join(" ");
  return (
    <svg width={size} height={size} viewBox={"0 0 "+size+" "+size}>
      {[1,2,3,4,5].map(function(l) {
        return <polygon key={l} points={gridPts(l)} fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="1"/>;
      })}
      {CRITERIA.map(function(_,i) {
        return <line key={i} x1={cx} y1={cy} x2={cx+r*Math.cos(angle(i))} y2={cy+r*Math.sin(angle(i))} stroke="rgba(255,255,255,0.1)" strokeWidth="1"/>;
      })}
      <polygon points={dataPts} fill={color+"44"} stroke={color} strokeWidth="2.5" strokeLinejoin="round"/>
      {CRITERIA.map(function(c,i) {
        const v = (scores[c.key]||0)/10;
        return <circle key={i} cx={cx+r*v*Math.cos(angle(i))} cy={cy+r*v*Math.sin(angle(i))} r="4" fill={color}/>;
      })}
      {CRITERIA.map(function(c,i) {
        const lr = r+24;
        return (
          <text key={i} x={cx+lr*Math.cos(angle(i))} y={cy+lr*Math.sin(angle(i))} textAnchor="middle" dominantBaseline="middle" fill="rgba(255,255,255,0.7)" fontSize="11" fontFamily="'Noto Sans JP',sans-serif">
            {c.emoji}
          </text>
        );
      })}
  ...
  return (
    <div style={{marginBottom:14}}>
      ...
      <div style={{position:"relative",height:6,background:"rgba(255,255,255,0.1)",borderRadius:3}}>
        ...
        <input type="range" min={0} max={10} step={1} value={value} onChange={function(e){onChange(Number(e.target.value));}}
...
function initScores() {
  return {aroma:5,umami:5,richness:5,spice:5,texture:5};
}
...
export default function App() {
  ...
  function handleSave() {
    if (!name.trim()) return;
    setSaving(true);
    const row = Object.assign({name:name.trim(),date:date,price:price,note:note}, scores);
    supabase.from("curry_record").insert([row]).select().then(function(res) {
      if (!res.error && res.data) setRecords(function(prev) { return prev.concat(res.data); });
  ...
  function handleDelete(id) {
    supabase.from("curry_record").delete().eq("id", id).then(function(res) {
      if (!res.error) {
  ...
  const sorted = records.slice().sort(function(a,b) {
    return totalScore(b,b.price) - totalScore(a,a.price);
  });
  ...
  return (
    <div style={{minHeight:"100vh",background:"radial-gradient(ellipse at 20% 0%,#3d0b00,#1a0500 50%,#0d0200)",fontFamily:"'Noto Sans JP',sans-serif",color:"#fff",paddingBottom:80}}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}>
          <div style={{background:"#1a0500",border:"1px solid rgba(232,93,4,0.4)",borderRadius:16,padding:"24px",maxWidth:320,width:"100%",textAlign:"center"}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={function(){setConfirmDelete(null);}} style={{flex:1,padding:"12px",background:"rgba(255,255,255,0.1)",border:"none",borderRadius:10,color:"#fff",fontSize:14,cursor:"pointer",fontFamily:"'Noto Sans JP',sans-serif"}}>キャンセル</button>
              <button onClick={function(){handleDelete(confirmDelete.id);}} style={{flex:1,padding:"12px",background:"#e85d04",border:"none",borderRadius:10,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"'Noto Sans JP',sans-serif"}}>削除する</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      <div style={{display:"flex",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
        {[["form","✏️ 採点"],["rank","🏆 ランキング"]].map(function(item) {
          const v = item[0]; const lb = item[1];
          return (
            <button key={v} onClick={function(){setView(v);}} style={{flex:1,padding:"12px 0",background:"none",border:"none",color:view===v?"#faa307":"rgba(255,255,255,0.4)",fontSize:13,fontWeight:view===v?700:400,borderBottom:view===v?"2px solid #faa307":"2px solid transparent",cursor:"pointer",fontFamily:"'Noto Sans JP',sans-serif"}}>{lb}</button>
          );
        })}
      </div>
      ...
      {view==="form" ? (
        <div style={{padding:"20px 18px"}}>
          <div style={{marginBottom:16}}>
            ...
            <input value={name} onChange={function(e){setName(e.target.value);}} placeholder="例：スパイスカレー 〇〇" style={inp}/>
          </div>
          <div style={{display:"flex",gap:10,marginBottom:20}}>
            <div style={{flex:1}}>
              <label style={lbl}>日付</label>
              <input type="date" value={date} onChange={function(e){setDate(e.target.value);}} style={Object.assign({},inp,{fontSize:13,padding:"11px 12px"})}/>
            </div>
            <div style={{flex:1}}>
              ...
              <div style={{position:"relative"}}>
                ...
                <input type="number" min={0} value={price} onChange={function(e){setPrice(e.target.value);}} placeholder="1200" style={Object.assign({},inp,{fontSize:13,padding:"11px 12px 11px 26px"})}/>
              </div>
            </div>
          </div>
          <div style={{background:"rgba(255,255,255,0.04)",borderRadius:16,padding:"14px",marginBottom:20,display:"flex",alignItems:"center"}}>
            ...
            <div style={{flex:1,textAlign:"center"}}>
              ...
              <div style={{fontSize:12,color:"rgba(255,255,255,0.4)",marginTop:4}}>
                基本 {baseScore(scores).toFixed(1)}
                {priceBonus(price)>0 ? <span style={{color:"#ffba08"}}> +{priceBonus(price).toFixed(1)}</span> : null}
              </div>
            </div>
          </div>
          ...
          <div style={{marginBottom:20}}>
            ...
            <textarea value={note} onChange={function(e){setNote(e.target.value);}} placeholder="感想・特徴など…" rows={2} style={Object.assign({},inp,{resize:"none"})}/>
          </div>
      ...
      {view==="rank" ? (
        <div style={{padding:"16px 18px"}}>
          {loading ? <div style={{textAlign:"center",color:"rgba(255,255,255,0.4)",marginTop:60}}>読み込み中…</div> : null}
          {!loading && sorted.length===0 ? <div style={{textAlign:"center",color:"rgba(255,255,255,0.3)",marginTop:60,fontSize:14}}>まだ採点がありません<br/>採点タブから追加してください 🍛</div> : null}
          {sorted.map(function(rec,idx) {
            const total = totalScore(rec,rec.price);
            const base = baseScore(rec);
            const bonus = priceBonus(rec.price);
            const isOpen = selected===rec.id;
            ...
            return (
              <div key={rec.id} style={{background:isOpen?"rgba(232,93,4,0.15)":"rgba(255,255,255,0.04)",border:"1px solid "+(isOpen?"rgba(232,93,4,0.4)":"rgba(255,255,255,0.08)"),borderRadius:14,marginBottom:10,overflow:"hidden",transition:"all 0.2s"}}>
                <div style={{display:"flex",alignItems:"center",padding:"14px 16px",gap:12,cursor:"pointer"}} onClick={function(){setSelected(isOpen?null:rec.id);}}>
                  ...
                  <div style={{textAlign:"right"}}>
                    ...
                    {bonus>0 ? <div style={{fontSize:10,color:"#ffba08",marginTop:2}}>{base.toFixed(1)} +{bonus.toFixed(1)}</div> : null}
                  </div>
                  <button onClick={function(e){e.stopPropagation();setConfirmDelete(rec);}}
                    style={{background:"rgba(255,255,255,0.08)",border:"none",borderRadius:8,color:"rgba(255,255,255,0.5)",fontSize:16,width:34,height:34,cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                    🗑️
                  </button>
                </div>
                {isOpen ? (
                  <div style={{padding:"0 16px 16px",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
                    ...
                    {rec.note ? (
                      <div style={{marginTop:10,padding:"10px 12px",background:"rgba(255,255,255,0.05)",borderRadius:8,fontSize:12,color:"rgba(255,255,255,0.55)",lineHeight:1.7}}>💬 {rec.note}</div>
                    ) : null}
                  </div>
                ) : null}
              </div>
